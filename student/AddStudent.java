...
public class AddStudent extends JDialog implements FocusListener, ActionListener {
        JLabel title, realsoft, logo, regdate, sdetails, edetails, cdetails, cfdetails;

        JLabel lmno, lmadate, lmname, lmgender, lmsmbno, lmpmbno, lmemail, lmpad, lmlad, lmdob, lmcity, lmsame; // student
                                                                                                                // details
        ...
        JPanel north = new JPanel(); // heading,logo,date
        JPanel center = new JPanel(); // form to enroll the student
        JScrollPane scrollPane; // JScrollPane for the center panel
        JPanel south = new JPanel(); // buttons to save , print
        Font font1, font2, font3; // fonts for heading,textfields and labels

        JButton bct, bcl, btr, bco, bbt; // bct --> button to add new city .... similarily --> bcl = college, btr=trade, co=course,bt=batch

        // Student details
        JTextField tmno, tmname1, tmname2, tmname3, tmemail, tmpad, tmlad; // tmpad=permanant address, tmplad=local address
        NTextField ntmsmbno, ntmpmbno; // ntmsmbno = student mobile number, ntmpmbno = parent mobile number
        JComboBox<String> cmcity; // select the cty
        JRadioButton m, f; // m --> male , f --> female
        ButtonGroup genderGroup;
        JCheckBox same; // for address local and permanant same
        NJDatePicker dob; // date of birth

        // Educational details
        JRadioButton rb1, rb2, rb3, rb4; // first,second,third,final year
        ButtonGroup bgacadyr;
        JComboBox<String> tmclnm, tmtrnm; // select college name,trade name

        // Course details
        JTextField ttime; // after selecting batch .... batch time will be automatically displayed
        JComboBox<String> cmconm, cmbtnm; // select the course name,batch name
        ButtonGroup bmattmode;
        JRadioButton online, offline; // attendence mode

        // Transaction details
        NTextField ntmcofee, ntmadmfee, ntmbalfee; // course fee,fees paid during admission,balance fees resp.
        JRadioButton cash, cheque, on; // on-->online
        JRadioButton account1, account2; // account to which money has been paid
        ButtonGroup btrntype, bmaccopt;
        JComboBox<String> cmtrdetail; // transaction details

        DB ref; // to store the refernce passed from the parent
        ResultSet rs; // to store the data retrieved from mysql tables
        int cnt = 0; // to count the number of students and give the cnt+1 number to newly enrolling student
        int recnt = 0; // to count the number of receipt and give the cnt+1 number to newly enrolling student

        AddCity act; // to add new city
        AddCollege acl; // to add new college
        AddTrade atr;
        AddCourse aco;
        AddBatch abt;
        AddStudent ast; // to store the refernce of current object

        JButton save, clear,back; // clear button to clear the typed data

        // Temparary Variables
        int sid, cofee, admfee, balfee, acyear;
        String fname, mname, lname, fullname, gender, smobile, pmobile, semail, speradd, sloadd, scity, sclname,
                        strname, sconame, sbatname, sbattime, smode, strtype, strdetail, stranto, formattedString;
        Date sdob, sdate;
        String tempdate;
        ...
        // Constructor
        public AddStudent(JFrame parent, String frmnm, boolean state, DB ref) {
                super(parent, frmnm, state);

                this.ref = ref; // stored the ref i.e. reference to DB class is stored in a temp variable

                ast = this; // stored the reference of this dialog into a temp variable

                ast.getContentPane().setBackground(Color.WHITE);
                ...
                //putting the logo image as a label
                try 
                {
                        logo = new JLabel();
                        ImageIcon imageIcon = new ImageIcon(new ImageIcon("D:\\realsoft\\utility\\logo.png").getImage()
                ...
                adate.dateField.setText("" + adate.getDate());
                ...
                cmconm = new JComboBox<String>();
                cmbtnm = new JComboBox<String>();
                ...
                cmtrdetail = new JComboBox<String>();
                ...
                center.add(ntmcofee);
                center.add(lmadmfee);
                center.add(ntmadmfee);
                center.add(lmbalfee);
                center.add(ntmbalfee);
                center.add(lmtrntype);
                center.add(cash);
                center.add(cheque);
                center.add(on);
                center.add(lmtrdetail);
                center.add(cmtrdetail);
                center.add(lmaccopt);
                center.add(account1);
                center.add(account2);
        ...
        public void actionPerformed(ActionEvent e) {
                ...
                if (b == save) {
                        try {
                                sid = Integer.parseInt("" + tmno.getText());
                                ...
                                ref.pstadd.setInt(1, sid);
                                ref.pstadd.setDate(2, sdate);
                                ref.pstadd.setString(3, fname);
                                ref.pstadd.setString(4, mname);
                                ref.pstadd.setString(5, lname);
                                ref.pstadd.setString(6, fullname);
                                ref.pstadd.setString(7, gender);
                                ref.pstadd.setDate(8, sdob);
                                ref.pstadd.setString(9, sclname);
                                ref.pstadd.setString(10, strname);
                                ref.pstadd.setInt(11, acyear);
                                ref.pstadd.setString(12, speradd);
                                ref.pstadd.setString(13, sloadd);
                                ref.pstadd.setString(14, scity);
                                ref.pstadd.setString(15, smobile);
                                ref.pstadd.setString(16, pmobile);
                                ref.pstadd.setString(17, semail);
                                ref.pstadd.setString(18, sconame);
                                ref.pstadd.setString(19, sbatname);
                                ref.pstadd.setString(20, sbattime);
                                ref.pstadd.setInt(21, cofee);
                                ref.pstadd.setInt(22, admfee);
                                ref.pstadd.setInt(23, admfee);
                                ref.pstadd.setInt(24, balfee);
                                ref.pstadd.setString(25, strtype);
                                ref.pstadd.setString(26, strdetail);
                                ref.pstadd.setString(27, stranto);
                                ref.pstadd.setString(28, smode);
                                ref.pstadd.setInt(29, 1);

                                ref.pstadd.executeUpdate();

                                
                                //Adding record to receipt
                                ref.preadd.setInt(1,recnt);
                                ref.preadd.setDate(2, sdate);
                                ref.preadd.setInt(3,sid);
                                ref.preadd.setInt(4,admfee);
                                ref.preadd.setString(5,strtype);
                                ref.preadd.setString(6,strdetail);
                                ref.preadd.setString(7, stranto);
                        ...
                        System.out.println("Emai id :" + semail);
                        ...
                        message = "Hello " + fname + " " + mname + " " + lname
                                        + " ,your admission in Realsoft Computers is confirmed." + "\n"
                                        + "Admission Date : " + adate.dateField.getText() + "\n" + "Batch Name: "
                                        + sbatname + " Timing : " + sbattime + "\n" + "Course Fees : " + cofee
                                        + "\nFee Paid : " + admfee + "\nBalance Fees : " + balfee + "\nThank you";
        ...
        public void focusGained(FocusEvent e) {
                ...
                if (t == tmname1) {
                        if (tmname1.getText().equals("First Name"))
                                tmname1.setText("");
                }
                ...
                if (t == tmname2) {
                        ...
                        if (tmname2.getText().equals("Middle Name"))
                                tmname2.setText("");
                ...
                if (t == tmname3) {
                        ...
                        if (tmname3.getText().equals("Last Name"))
                                tmname3.setText("");
                }
